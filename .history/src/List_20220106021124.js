...
const EditBtn = (props) => {
  return (
    <button
      ...
      onClick={() => props.editMode(props.elementId)}
    >
      Edit
    </button>
  );
...
class List extends Component {
  ...
  listBasedOnStatus(list, status = null) {
    ...
    if (status) {
      ...
      items = list.map((row, index) => {
        let checked = row.status === "completed" ? "defaultChecked" : "";
        return this.listItemEditMode(row, index, checked);
      });
    }
    return items;
  }
    ...
    return (
      ...
      >
        ...
        <div className="float-end">
          <EditBtn />
          <DeleteBtn elementId={row.id} handleDelete={this.props.handleDelete} />
        </div>
      </li>
    );
  }
  ...
  listItemEditMode(row, index, checked) {
    return (
      ...
      >
        <div class="d-flex">
          <input type="text" name="" id="" className="form-control mr-2"/>
